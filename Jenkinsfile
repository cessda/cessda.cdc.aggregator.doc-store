...
node(node_label) {
    ...
    def image_tag = "${DOCKER_ARTIFACT_REGISTRY}/cdc-agg-docstore:${env.BRANCH_NAME.toLowerCase().replaceAll('[^a-z0-9\\.\\_\\-]', '-')}-${env.BUILD_NUMBER}"
    ...
    tasks_1['Prepare Pylint, Run Analysis, Archive & Publish report'] = {
        docker.image('python:3.9').inside('-u root') {
            ...
            stage('Run PyLint') {
                ...
                sh """
                . ./${pylintEnvName}/bin/activate
                pylint -f parseable cdcagg_docstore | tee ${pylint_report_path}
                """
            }
